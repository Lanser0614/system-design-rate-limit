...
network:
	  docker network create cbs_core || true

composer-install:
	docker exec blatnoy_app composer install

composer-update:
	docker exec blatnoy_app composer update

composer-require:
	docker exec blatnoy_app composer require $(name)


composer-remove:
	docker exec blatnoy_app composer remove $(name)

cp-env:
	docker exec blatnoy_app cp .env.example .env

artisan-key-generate:
	docker exec blatnoy_app  php artisan key:generate

artisan-moonshine-create-user:
	docker exec blatnoy_app  php artisan moonshine:user


laravel:
	docker exec blatnoy_app  php artisan $(name)

artisan-migrate:
	docker exec blatnoy_app  php artisan migrate

project-cs-check:
	docker exec blatnoy_app  ./vendor/bin/php-cs-fixer fix -vvv --dry-run --show-progress=dots --config=./docker/cs-fixer/.php-cs-fixer.php --allow-risky=yes

project-cs-fix:
	docker exec blatnoy_app ./vendor/bin/php-cs-fixer fix -vvv --show-progress=dots --config=./docker/cs-fixer/.php-cs-fixer.php --allow-risky=yes

cs-fix-path:
	docker exec blatnoy_app ./vendor/bin/php-cs-fixer fix --path-mode=intersection $(path) -vvv --show-progress=dots --config=./docker/cs-fixer/.php-cs-fixer.php --allow-risky=yes

phpunit:
	docker exec blatnoy_app php artisan test --parallel --processes=4

phpunit-path:
	docker exec blatnoy_app ./vendor/bin/phpunit  --filter  $(path)

phpstan:
	docker exec blatnoy_app ./vendor/bin/phpstan analyse --memory-limit=2G
##
# up
#
